...
import { CarbonSequesteredWidget } from './CarbonSequesteredWidget';

export const CarbonSequestered: React.FC = () => {
  ...
  const content = (
    <BaseRow gutter={[32, 32]}>
      <BaseCol span={24}>
        <BaseForm.Title>{t('My Total Carbon Sequestration')}</BaseForm.Title>
      </BaseCol>
      <BaseCol span={24}>
        <BaseSpin spinning={loading}>
          <CarbonSequesteredWidget cards={cards} onCardRemove={handleCardRemove} onCardAdd={handleCardAdd} />
        </BaseSpin>
      </BaseCol>
    </BaseRow>
  );
